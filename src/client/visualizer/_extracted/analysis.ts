...
export function detectPitch(fftData, sampleRate) {
	let maxValue = -Infinity;
	let maxIndex = 0;

	const minIndex = Math.floor((20 * fftData.length) / (sampleRate / 2));
	const maxIndex2 = Math.floor((18000 * fftData.length) / (sampleRate / 2));

	for (let i = minIndex; i < maxIndex2; i++) {
...
export function calculateColor(frequency, amplitude, saturationBoost) {
